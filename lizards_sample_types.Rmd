...
workingdir="~/Documents/Projects/Lizards_Mauricio"
counts_file="./genome.count.tsv"
tree_file="./gtdbtk.backbone.bac120.classify.tree"
taxonomy_file="./gtdbtk.summary.tsv"
metadata_file="./metadata.csv"
coverage_file="./genome.covered_bases.tsv"
genome_lenght="./genome.length.tsv"
annotations_file="./annotations.tsv"
contam_comp="./quality_report.tsv"
...
## Load data 
setwd(workingdir)
counts_table <- read_tsv(counts_file) %>%
    dplyr::rename(Genome=1)
basehits <- read_tsv(coverage_file) %>%
    dplyr::rename(Genome=1)
maglengths <- read_tsv(genome_lenght) %>%
...
mags_table <- read_tsv(taxonomy_file)%>%
    dplyr::rename(Genome=1)
contam_comp_table <- read_tsv(contam_comp) %>%
    dplyr::rename(Genome=1)
magtree <- read.tree(tree_file)
...
filtered_df <- read_tsv(annotations_file)
...
maglengths_row <- column_to_rownames(maglengths, "Genome")
genome_read_sizes <- maglengths %>%
    mutate(MAGlength = MAGlength / 150) %>%
    pull()
...
#  stat_compare_means() +
    theme(axis.text.x = element_text(vjust = 0.5), #, size=18
...
Model_rich <- lme(fixed = richness ~ Sample_type, data = alpha_div_N,
               random = ~ 1 | Individual)
summary(Model_rich)
```
...
Model_rich <- lme(fixed = neutral ~ Sample_type, data = alpha_div_N,
               random = ~ 1 | Individual)
summary(Model_rich)
```
...
Model_rich <- lme(fixed = phylo ~ Sample_type, data = alpha_div_N,
               random = ~ 1 | Individual)
summary(Model_rich)
```
