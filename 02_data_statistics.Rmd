...
data_stats %>%
  ...
  pivot_longer(-sample) %>%
  separate(
    col = "sample", into = c("sample", "tissue"), sep = "\\.", remove = FALSE
  ) %>%
  ...
  ggplot(aes(x = sample, y = value, fill = name)) +
      ...
      theme_minimal() +
      labs(y="DNA sequence fraction",x="Samples",fill="Sequence type")
```

```{r data_estimations_plot, message=FALSE, warning=FALSE, fig.height=10, fig.width=10, fig.fullwidth=TRUE}
data_stats %>%
  mutate(
    unmapped_reads = trimmed_reads - mapped_SceUnd - mapped_mags,
