...
reads_fastqc <-
  read_tsv("resources/report/by_step/reads_data/multiqc_fastqc.txt") %>%
  clean_names() %>%
  select(
...
reads_general <-
  read_tsv("resources/report/by_step/reads_data/multiqc_general_stats.txt") %>%
  clean_names() %>%
...
fastp_fastqc <-
  read_tsv("resources/reportsby_step/fastp_data/multiqc_fastqc.txt") %>%
  clean_names() %>%
  select(
...
fastp_general <-
  read_tsv("resources/reportsby_step/fastp_data/multiqc_general_stats.txt") %>%
  clean_names() %>%
  rename_with(~str_remove(., "fast_qc_mqc_generalstats_fastqc_"))
...
human <-
  read_tsv("resources/reportsby_step/bowtie2_host_Homo_sapiens.GRCh38_data/multiqc_general_stats.txt") %>%
  clean_names() %>%
