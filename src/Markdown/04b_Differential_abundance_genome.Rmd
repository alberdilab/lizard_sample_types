...
```{r nozero_phylo, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
structural_zeros <- genome_counts %>%
  ...
  mutate(all_zeros_feces = all(c_across(all_of(feces_samples)) == 0)) %>% # set true if all samples in TJ1 have zeros
  mutate(all_zeros_cloaca = all(c_across(all_of(cloaca_samples)) == 0)) %>% # set true if all samples in TJ2 have zeros
  mutate(average_feces = mean(c_across(all_of(feces_samples)), na.rm = TRUE)) %>% # get average genome counts across TJ1
  mutate(average_cloaca = mean(c_across(all_of(cloaca_samples)), na.rm = TRUE)) %>% # get average genome counts across TJ2
...
diagdds <- phyloseq_to_deseq2(genome_data, ~Sample_type)
...
diagdds <- phyloseq_to_deseq2(physeq_genome_filtered, ~Sample_type)
...
```{r wilcox, comment="", echo=FALSE, message=FALSE, warning=FALSE}
physeq_clr <- microbiome::transform(physeq_genome_filtered, "clr")
...
library(ALDEx2)
genome_counts_aldex <- genome_counts %>%
  filter(!genome %in% structural_zeros$genome) %>% # remove structural zeros
...
phylum_colors <- read_tsv("https://raw.githubusercontent.com/earthhologenome/EHI_taxonomy_colour/main/ehi_phylum_colors.tsv") %>%
  mutate_at(vars(phylum), ~ str_replace(., "[dpcofgs]__", "")) %>%
  right_join(genome_counts_filtered.all, by = join_by(phylum == phylum)) %>%
  arrange(match(genome, bacteria_tree$tip.label)) %>%
  dplyr::select(phylum, colors) %>%
  mutate(colors = str_c(colors, "80")) %>% # add 80% alpha
  unique() %>%
...
genome_counts_filtered.all %>%
  ...
  scale_color_manual(values = phylum_colors) +
  geom_text(aes(-max(genome_counts_filtered.all$effect) + 1, 0.7), label = "Enriched\nin feces", color = "#666666") +
  geom_text(aes(max(genome_counts_filtered.all$effect) - 1, 0.7), label = "Enriched\nin cloaca", color = "#666666") +
  theme(
    legend.position = "none",
    panel.background = element_blank(),
...
tse <- mia::makeTreeSummarizedExperimentFromPhyloseq(genome_data)
