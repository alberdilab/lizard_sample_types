...
# Generate  basal tree
circular_tree <- bacteria_tree %>%
  force.ultrametric(method = "extend", message = FALSE) %>% # extend to ultrametric for the sake of visualisation
...
function_tree <-
  function_tree %>%
  gheatmap(phylum_heatmap, offset = 0, width = 0.1, colnames = FALSE) +
  scale_fill_manual(values = phylum_colors) +
  labs(fill = "Phylum")
...
function_tree <- function_tree +
  ...
  theme(legend.text = element_text(size = 8), title = element_text(size = 10), legend.key.size = unit(0.3, "cm"), )
