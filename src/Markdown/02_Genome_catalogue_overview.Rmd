---
title: "Genome catalogue overview"
...
# Generate  basal tree
circular_tree <- force.ultrametric(bacteria_tree, method = "extend") %>% # extend to ultrametric for the sake of visualisation
...
circular_tree <- circular_tree +
  ...
  geom_fruit(
    data = genome_metadata,
    geom = geom_bar,
    mapping = aes(x = Genome_Size, y = genome),
    offset = 0.05,
    orientation = "y",
    stat = "identity"
  )
...
genome_biplot <- genome_metadata %>%
  select(c(genome, domain, phylum, Completeness, Contamination, Genome_Size)) %>%
  arrange(match(genome, rev(bacteria_tree$tip.label))) %>% # sort MAGs according to phylogenetic tree
...
grid.arrange(
  grobs = list(genome_completeness, genome_biplot, genome_contamination),
  layout_matrix = rbind(
...
# Functional overview
```{r funcitonal, comment="", echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# Aggregate basal GIFT into elements
function_table <- genome_gifts %>%
  to.elements(., GIFT_db)

# Generate  basal tree
function_tree <- force.ultrametric(bacteria_tree, method = "extend") %>%
  ggtree(., size = 0.3)
```

```{r functional_tree, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
function_tree <- function_tree +
  geom_fruit(
    ...
    mapping = aes(x = Genome_Size, y = genome, color = Completeness),
    offset = 3.8,
    orientation = "y",
    stat = "identity"
  ) +
  scale_color_gradient(low = "#cf8888", high = "#a2cc87") +
  labs(color = "Genome completeness") + theme(legend.text = element_text(size = 8), title = element_text(size = 10), legend.key.size = unit(0.3, "cm"), )

function_tree
```

# Functional ordination
```{r tSNE_function, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
function_ordination <- tSNE_function$Y %>%
  ...
  select(genome, phylum, tSNE1, tSNE2, Genome_Size) %>%
  ggplot(aes(x = tSNE1, y = tSNE2, color = phylum, size = Genome_Size)) +
